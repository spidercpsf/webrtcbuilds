...
function checkout() {
  ...
  if [[ -n "$prev_revision" && "$revision" != "$prev_revision" ]]; then
    echo The target revision has changed. sync sources for the new code
...
video_capture_external.o|device_info_external.o"
  ...
  echo "Generating Xcode $gn_args $outputdir"
  gn gen $outputdir --args="$gn_args enable_dsyms=true ffmpeg_branding=\"Chrome\" proprietary_codecs=true" --ide=xcode
  echo "Generated Xcode $gn_args $outputdir"
  pushd $outputdir >/dev/null
  ninja -C .
